...
if [ -f $STEP.done ]; then
    ...
    ln -sf $bam $outstem.sorted.bam >$STEP.log 2>&1 \
    && touch  $STEP.done
...
if [ -f $STEP.done ]; then
    ...
    $gatk --java-options '-Xmx12g -Xms12g  -XX:ParallelGCThreads=2' GenotypeGVCFs -O $outstem.genotyped.g.vcf -R $genome -V $outstem.g.vcf.gz -all-sites true > $STEP.log 2>&1 \
    && touch  $STEP.done

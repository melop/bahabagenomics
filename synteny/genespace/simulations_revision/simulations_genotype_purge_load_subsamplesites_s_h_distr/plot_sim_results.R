setwd("/public3/group_crf/home/cuirf/bahaha_assembly/synteny/genespace/simulations_revision/simulations_genotype_purge_load_subsamplesites_s_h_distr");
...
datSimsToPlot <- datSimDefs[datSimDefs$SeedingPopSize %in% c(10) & datSimDefs$PopSize %in% c(100) & datSimDefs$h == 0.5 & datSimDefs$h_sd>=0 & datSimDefs$s_sd >= 0 & datSimDefs$s %in% c(0, 0.001) & datSimDefs$TopPercentileForBreeding %in% c(1, 0.25) & datSimDefs$GenotypedMarkerPerc %in% c(1, 0.25,0.05) & datSimDefs$SDDel==-1, ]
sOut <- "plot_neteffect_s0.001.pdf";

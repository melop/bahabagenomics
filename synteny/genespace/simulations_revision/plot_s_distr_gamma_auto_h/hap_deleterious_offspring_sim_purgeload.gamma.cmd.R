...
#this script sets s using a gamma distribution, and setting h automatically according to https://academic.oup.com/evolut/article/77/3/690/6936536
...
s_sd <- as.numeric(args[15]); #the gamma shape parameter of s to draw from
...
arrSelectionCoef <- rgamma(n=nTotalRows, shape = s_sd, scale= s/s_sd ); #this simulates s with gamma distribution with mean of s, shape of s_sd
cat("Simulated gamma mean=", s, " shape = ", s_sd, "\n");
arrSelectionCoef[arrSelectionCoef<arrSLimits[1]] <- arrSLimits[1];
arrSelectionCoef[arrSelectionCoef>arrSLimits[2]] <- arrSLimits[2];

arrDominanceEffect <- rep(0.5, length(arrSelectionCoef))
arrDominanceEffect[arrSelectionCoef>0.05] <- 0;
arrDominanceEffect[arrSelectionCoef<=0.05] <- 0.5-arrSelectionCoef[arrSelectionCoef<=0.05]*10;
...
cat("Using multiplicative fitness model\n")
fnFitness <- function(datInd) {
  ...
  #multiplicative
  return(max(prod(1-arrHetSites * arrHetPerSiteFitness) * prod(1-arrHomPerSiteFitness * arrHomDelSites) , 0));
}
